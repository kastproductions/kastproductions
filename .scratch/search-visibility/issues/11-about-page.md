...
- "He owns the agents we build.": the same paragraph.
...
- "Companies Karolis Stulgys has shipped for, across four continents" and the seventeen names with their links: `clients` and its comment ("Seventeen companies ... across four continents"), and the home page's "has shipped for the companies listed above". Naming him rather than "we" is the attribution the `plans/README.md` finding asks for ("The clients strip credits the company with the founder's earlier work"). "seventeen" in the description is the length of `clients`.
- "Six references, quoted as written" and "People who worked with Karolis Stulgys wrote these about his earlier work. Nothing in them is edited.": `references` and its comment ("quoted as written. Do not edit the quotes."); "earlier work" is the wording of the `plans/README.md` finding and of the brief. The six quotes, names and positions are rendered from `references` with the same markup the home page used, so they are byte-identical to what it printed; `tests/about.test.ts` reads each quote back out of the export.
- Description: "Karolis Stulgys founded KastProductions in Vilnius and signs the merges we make. Seventeen companies he has shipped for, and six references quoted as written.": the sources above.

**Review.** The two-axis review (standards, spec) found no hard breach. Acted on: the clients strip is now `src/components/clients-strip.tsx`, drawn on both pages; each profile URL is parsed once; the README's `public/reviewers/` line names the about page; three sentences that outran their sources were cut back ("One engineer answers for every agent" had no source for a headcount; "signs every merge" contradicted the terms' self-run plan, so it is "signs the merges we make"; "before KastProductions" said more than the finding's "earlier work"). Not acted on: hard-coded counts ("seventeen", "six") follow the home page's "four continents" precedent; the header-region loop in `tests/about.test.ts` mirrors `tests/legal.test.ts` and stays a copy rather than a helper in `tests/export.ts`, which four siblings append to at once; moving the references off the home page was flagged as beyond the acceptance boxes and is kept as the recorded decision below.
