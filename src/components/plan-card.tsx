 ...
 * rather than in four page files. `lead` sets the card on ink, which is how
 * the price list marks the plan it leads with.
 */
export function PlanCard({
  ...
  return (
    <Card className={cn(lead && "dark border-foreground")} size="lg">
      <CardHeader>
      ...
      <CardContent className="mt-auto">
        <div className="flex flex-col gap-1 border-t border-hairline pt-4">
          {prices.map((price) => (
            <div
              className="font-heading text-2xl leading-tight font-extrabold tracking-[-0.03em] tabular-nums"
              key={price.per}
            >
              {price.amount}{" "}
              <span className="mt-0.5 block font-sans text-[0.82rem] font-medium tracking-normal text-faint">
                {price.per}
              </span>
            </div>
          ))}
        ...
        {includes.length > 0 ? (
          <ul className="mt-4 grid gap-2 text-[0.86rem] leading-normal text-prose *:relative *:pl-4 *:before:absolute *:before:top-[0.7em] *:before:left-0 *:before:h-px *:before:w-1.75 *:before:bg-signal">
            {includes.map((item) => (
              <li key={item}>{item}</li>
            ))}
          </ul>
      ...
      <CardFooter>
        <a className={cn(buttonVariants({ variant: "outline" }), "w-full")} href={href}>
          {cta}
        </a>
      </CardFooter>
    </Card>
