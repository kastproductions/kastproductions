...
function Accordion({ className, ...props }: AccordionPrimitive.Root.Props) {
  return (
    <AccordionPrimitive.Root
      data-slot="accordion"
      className={cn("flex w-full flex-col border-t", className)}
      {...props}
    />
  )
}
    ...
    <AccordionPrimitive.Header className="flex">
      ...
      >
        ...
        <ChevronDownIcon
          ...
          className="pointer-events-none shrink-0 transition-transform duration-300 ease-settle group-aria-expanded/accordion-trigger:rotate-180"
        />
      </AccordionPrimitive.Trigger>
...
function AccordionContent({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "h-(--accordion-panel-height) max-w-(--measure) pt-0 pb-5 text-base leading-relaxed text-prose data-ending-style:h-0 data-starting-style:h-0 [&_a]:underline [&_a]:underline-offset-3 [&_a]:hover:text-foreground [&_p:not(:last-child)]:mb-4",
          className
        )}
      >
        {children}
